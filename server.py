#!/usr/bin/env python3
"""
poker_server.py  —  Servidor de Texas Hold'em para El Farol Rojo
================================================================
Uso:
    python3 poker_server.py [--port 5555] [--min-players 2] [--max-players 6]

Protocolo: JSON sobre TCP, un objeto por línea (\n).

Cliente  →  Servidor
────────────────────
{"type": "join",   "name": "Pepe"}
{"type": "action", "action": "fold|call|check|raise", "amount": 0}

Servidor  →  Cliente
────────────────────
{"type": "lobby",         "players": ["Pepe", "Ana"]}
{"type": "deal",          "hand": [[v,p],[v,p]], "pot": X, "blind": Y,
                          "my_money": Z, "players": [{name,money,folded},...]}
{"type": "community",     "cards": [[v,p],...], "pot": X}
{"type": "your_turn",     "street": "pre_flop", "pot": X, "to_call": Y}
{"type": "player_action", "player": "Ana", "action": "call", "amount": 0,
                          "pot": X, "players": [...]}
{"type": "result",        "winner": "Pepe", "hand_name": "Pareja",
                          "pot": X, "my_money": Z, "players": [...]}
{"type": "next_hand"}
{"type": "error",         "msg": "..."}
{"type": "waiting",       "msg": "Esperando más jugadores..."}
"""
...
def best_hand(cards):
    """Mejor mano de 5 entre todas las combinaciones de las cartas dadas."""
    best = None
...
class Player:
    def __init__(self, conn, addr, name, money):
        self.conn   = conn
        self.addr   = addr
        self.name   = name
        self.money  = money
...
class PokerRoom:
    ...
    def _players_info(self):
        return [{'name': p.name, 'money': p.money, 'folded': p.folded}
                for p in self.players]

    def broadcast_lobby(self):
        msg = {'type': 'lobby', 'players': [p.name for p in self.players]}
    ...
    def add_player(self, conn, addr, name):
        with self.lock:
            if len(self.players) >= self.max_players:
                try:
                    conn.sendall((json.dumps({'type':'error','msg':'Sala llena'}) + '\n').encode())
                    conn.close()
                except Exception:
                    pass
                return None
            ...
            while name in existing:
                name = f"{orig}{i}"; i += 1
            p = Player(conn, addr, name, self.STARTING_MONEY)
    ...
    def _run_game(self):
        ...
        try:
            while True:
                ...
                if len(active) < 2:
                    print("[GAME] Quedan menos de 2 jugadores. Fin.")
                    break
                self._play_hand()
    ...
    def _play_hand(self):
        ...
        for street in self.STREETS:
            ...
            while True:
                ...
                while time.time() < deadline:
                    ...
                    self._action_event.clear()
                    if self._pending_action and self._pending_action[0] == next_actor.name:
                        break
                    if not next_actor.active:
                        self._pending_action = (next_actor.name, 'fold', 0)
                        break

                if self._pending_action is None:
                    if p_call_needed == 0:
                        self._pending_action = (next_actor.name, 'check', 0)
                    else:
                        self._pending_action = (next_actor.name, 'fold', 0)
        ...
        for p in non_folded:
            ...
            score, hand_name = best_hand(all_cards)
        ...
        winner_p  = winners[0][1]
        hand_name = winners[0][2]

        for p in active_players:
            p.send({
                'type':      'result',
                'winner':    winner_p.name,
                'hand_name': hand_name,
                'pot':       self.pot,
                'my_money':  p.money,
                'players':   self._players_info(),
...
def handle_client(conn, addr, room: PokerRoom):
    buf = ""
    player = None
    try:
        while True:
            ...
            while '\n' in buf:
                ...
                if mtype == 'join' and player is None:
                    name = str(msg.get('name', 'Jugador'))[:20]
                    player = room.add_player(conn, addr, name)
...
def main():
    parser = argparse.ArgumentParser(description="Servidor de Poker — El Farol Rojo")
    parser.add_argument('--port',        type=int, default=5555,  help='Puerto TCP (default: 5555)')
    parser.add_argument('--min-players', type=int, default=2,     help='Jugadores mínimos para empezar')
    parser.add_argument('--max-players', type=int, default=6,     help='Jugadores máximos por sala')
    ...
    srv.listen(16)
    print(f"[SERVER] Escuchando en 0.0.0.0:{args.port} "
          f"(mín {args.min_players}, máx {args.max_players} jugadores)")
